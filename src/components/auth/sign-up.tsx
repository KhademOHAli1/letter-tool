/**
 * SignUp component - Account registration
		...
		return (
			<Card className="w-full max-w-md">
				<CardHeader className="space-y-1">
					...
					<CardDescription>
						Account registration is currently not configured for this
						deployment.
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="rounded-md bg-yellow-50 p-4 text-sm text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300">
						...
						<p className="mt-1 text-yellow-700 dark:text-yellow-400">
							...
							NEXT_PUBLIC_SUPABASE_ANON_KEY) for registration to work.
						</p>
					</div>
				</CardContent>
				<CardFooter>
	...
	return (
		<Card className="w-full max-w-md">
			<CardHeader className="space-y-1">
				...
				<CardDescription>
					Sign up to create and manage advocacy campaigns
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-4">
				{error && (
			...
			<CardFooter className="flex justify-center">
				<p className="text-sm text-muted-foreground">
					...
					<Link
						href={`/auth/sign-in${redirect !== "/admin" ? `?redirect=${redirect}` : ""}`}
